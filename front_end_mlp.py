...
def process_data(input):
    ...
    answer = np.array([[np.max(matrix[:, 0])], [np.min(matrix[:, 1])], [np.mean(matrix[:, 2])]]) # High, low and close
...
def generate_data(input_date):
    ...
    timesteps = 10 # The model only works for 10 timesteps for now
    ...
    if (input_date != latest_date_avaiable):
      while(latest_date_avaiable < input_date):
        ...
        latest_input_query_initial.iloc[0:timesteps, :] = latest_input_query_initial.iloc[1:timesteps+1, :]
        latest_input_query_initial.iloc[timesteps+1:timesteps+2, :] = predictions
        latest_input_query = createTimeSeriesData(latest_input_query_initial, timesteps)
        latest_input_query = latest_input_query.iloc[:, :-8].values 
